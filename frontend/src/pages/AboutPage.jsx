    ...
    <article className="mx-auto max-w-4xl">
      ...
      <div className="grid gap-8 md:grid-cols-[1fr_1.35fr]">
        ...
        <div className="space-y-7 text-[15px] leading-7 text-slate-600 dark:text-slate-300">
          ...
          <section className="rounded-xl border-l-4 border-orange bg-white p-5 shadow-card dark:bg-navy-900"><h2 className="mb-2 font-brand text-2xl font-semibold text-navy dark:text-slate-100">{t("El origen del nombre")}</h2><p><strong className="text-navy dark:text-white">AlzoLab</strong> {t("une")} <strong className="text-navy dark:text-white">Alzo</strong>, {t("tomado de Alzola, con")} <strong className="text-navy dark:text-white">lab</strong>, {t("laboratorio. La sílaba")} <span className="font-semibold text-orange">la</span> {t("actúa como bisagra visual: completa el apellido Alzo·la y, al mismo tiempo, abre la palabra “lab”. El nombre convierte una firma personal en la idea central del programa: un laboratorio donde experimentar con corpus.")}</p></section>
        </div>
      </div>
    </article>
  );
